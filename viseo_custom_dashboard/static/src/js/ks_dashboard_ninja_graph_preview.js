odoo.define('ks_dashboard_ninja_list.ks_dashboard_graph_preview', function(require) {
    ...
    var KsGraphPreview = AbstractField.extend({
        ...
        _render: function() {
            ...
            if (this.recordData.ks_dashboard_item_type !== 'ks_tile' && this.recordData.ks_dashboard_item_type !== 'ks_kpi' && this.recordData.ks_dashboard_item_type !== 'ks_list_view') {
                if (this.recordData.ks_model_id) {
                    if (this.recordData.ks_chart_groupby_type == 'date_type' && !this.recordData.ks_chart_date_groupby) {
                        ...
                        this.$el.append($('<div>').text("Veuillez choisir une unité de mesure et un groupe"));
                    ...
                    this.$el.append($('<div>').text("Veuillez choisir un modèle"));
                }
            }

            // else if (this.recordData.ks_dashboard_item_type == 'ks_bar_chart') {
            //     if (this.recordData.ks_model_id_1_many && this.recordData.ks_model_id_2_many) {
            //         if (this.recordData.ks_chart_relation_groupby_many == 'date_type' && !this.recordData.ks_chart_date_groupby) {
            //             return this.$el.append($('<div>').text("Select Group by date to create chart based on date groupby"));
            //         } 
            //         else if (this.recordData.ks_chart_data_count_type === "count" && !this.recordData.ks_chart_relation_groupby_many) {
            //             this.$el.append($('<div>').text("Select Group By to create chart view"));
            //         } 
            //         else if (this.recordData.ks_chart_data_count_type !== "count" && 
            //                 ( (this.recordData.ks_chart_measure_field_many.count === 0 && this.recordData.ks_chart_measure_field_2_many.count === 0) || !this.recordData.ks_chart_relation_groupby_many)) {
            //             this.$el.append($('<div>').text("Veuillez choisir une unité de mesure et un groupe"));
            //         }
            //         else if (!this.recordData.ks_chart_data_count_type) {
            //             this.$el.append($('<div>').text("Select Chart Data Count Type"));
            ...
            //     } else {
            //         //"Select a Model first."
            //         this.$el.append($('<div>').text("Veuillez choisir les 2 modèles"));
            //     }
            // }
        },
        ...
        renderChart: function() {
            var self = this;
            if (this.recordData.ks_dashboard_item_type === 'ks_bar_chart') {
                var self = this;
                var scales = {}
                scales.yAxes = [{
                        type: "linear",
