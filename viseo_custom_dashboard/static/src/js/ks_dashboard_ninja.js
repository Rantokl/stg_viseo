odoo.define('viseo_custom_dashboard.ks_dashboard', function(require) {
	"use strict";
	//  changing show gridlines to false
	...
	var first_widget = false
	var edit_options = false
	...
	var KsDashboardNinja = AbstractAction.extend({
		...
		events: {
			...
			'click .ks_dashboard_widget_options': '_onKsWidgetOptionsClick',
			'mouseenter .ks_chart_card_body ': '_onKsChartHover',
			'mouseenter .ks_dashboard_item_main_body ': '_onKsItemHover',
			'mouseenter .ks_dashboard_item_main_body_l2 ': '_onKsItemHover_l2',
			'mouseenter .ks_dashboard_item_main_body_l5 ': '_onKsItemHover_l5',
			'mouseleave .ks_chart_card_body ': '_onKsItemOut',
			'mouseleave .ks_dashboard_item_main_body ': '_onKsItemOut',
			'mouseleave .ks_dashboard_item_main_body_l2 ': '_onKsItemOut',
			'mouseleave .ks_dashboard_item_main_body_l5 ': '_onKsItemOut',
		...
		start: function() {
			...
			this._rpc({
				...
				method: 'get_dashboard_color',
				args: [
					this.ks_dashboard_id
				],
				
			}).then(function(new_item_data) {
				this.$el[0].style.background = this.convertHex(new_item_data.split(',')[0],new_item_data.split(',')[1]) 
				this.color = new_item_data
				...
				if (self.grid) {
					if (items[i].ks_dashboard_item_type === 'ks_tile') {
						...
						if (items[i].id in self.gridstackConfig) {
							self.grid.addWidget($(item_view), self.gridstackConfig[items[i].id].x, self.gridstackConfig[items[i].id].y, self.gridstackConfig[items[i].id].width, self.gridstackConfig[items[i].id].height, false, 1, null, 1, 2, items[i].id);
						} else {
							self.grid.addWidget($(item_view), 0, 0, 8, 2, true, 1, null, 1, 2, items[i].id);
						}
						...
						if (items[i].id in self.gridstackConfig) {
							self.grid.addWidget($kpi_preview, self.gridstackConfig[items[i].id].x, self.gridstackConfig[items[i].id].y, self.gridstackConfig[items[i].id].width, self.gridstackConfig[items[i].id].height, false, 1, null, 1, 3, items[i].id);
						} else {
							self.grid.addWidget($kpi_preview, 0, 0, 3, 2, true, 1, null, 1, 1, items[i].id);
						}
		...
		ksNumFormatterCustom: function(num, digits) {
			var negative;
			var si = [{
					value: 1,
					symbol: ""
		...
		_renderGraph: function(item) {
			...
			var chart_id = item.id, chart_title = item.name;
			...
			if (chart_id in self.gridstackConfig) {
				self.grid.addWidget($ks_gridstack_container, self.gridstackConfig[chart_id].x, self.gridstackConfig[chart_id].y, self.gridstackConfig[chart_id].width, self.gridstackConfig[chart_id].height, false, 1, null, 1, null, chart_id);
			} else {
				self.grid.addWidget($ks_gridstack_container, 0, 0, 13, 4, true, 1, null, 1, null, chart_id);
			}
		...
		_renderChart: function($ks_gridstack_container, item) {
			...
			// Calculate font size based on the size of the great-grandparent element

			console.log($ks_gridstack_container.find('.card-body').height())
			console.log($ks_gridstack_container.find('.card-body').width())

			var ks_chart_x_axe_font_size = item.ks_chart_x_axe_font_size * 1.33333;
			var ks_chart_y_axe_font_size = item.ks_chart_y_axe_font_size * 1.33333;
			var ks_chart_legend_font_size = item.ks_chart_legend_font_size * 1.33333;

			var responsivLabelFontSize = 12;

			var scales = {}
			scales.xAxes = [{
				ticks: {
					fontSize: ks_chart_x_axe_font_size,
				}
			}]
			...
			if (chart_data["ks_show_second_y_scale"] && item.ks_dashboard_item_type === 'ks_bar_chart') {
				scales.yAxes = [{
						...
						ticks: {
							fontSize: ks_chart_y_axe_font_size,
						}
					},
					{
						type: "linear",
						...
						ticks: {
							...
							callback: function(value, index, values) {
								...
								if (ks_selection === 'monetary') {
									...
									var ks_data = self.ksNumFormatterCustom(value, 1);
									ks_data = KsGlobalFunction.ks_monetary(ks_data, ks_currency_id);
									return ks_data;
								} else if (ks_selection === 'custom') {
									var ks_field = chart_data.ks_field;
									return self.ksNumFormatterCustom(value, 1) + ' ' + ks_field;
								} else {
									return self.ksNumFormatterCustom(value, 1);
								}
							}
						}
					}
			...
			else if (chart_data["ks_show_second_y_scale"] && item.ks_dashboard_item_type === 'ks_bar_chart_many') {
				scales.yAxes = [{
						type: "linear",
						display: true,
						position: "left",
					...
					},
					{
						...
						ticks: {
							...
							fontSize: ks_chart_y_axe_font_size,
							callback: function(value, index, values) {
								if (chart_data.ks_monetary_unit_2_many) {
									var ks_currency_id = chart_data.ks_monetary_unit_2_many;
									var ks_data = self.ksNumFormatterCustom(value, 1);
									ks_data = KsGlobalFunction.ks_monetary(ks_data, 105);
									return ks_data;
								} else {
									return self.ksNumFormatterCustom(value, 1);
								}
			...
			var ksMyChart = new Chart($ksChartContainer[0], {
				...
				options: {
					...
					legend: {
						labels: {
							fontSize: ks_chart_legend_font_size
						}
					},
					// *************************************************************
					plugins: {
			...
			$('.ks_chart_heading').css("font-size", item.ks_chart_title_font_size);
		},
		...
		ksChartColors: function(palette, ksMyChart, ksChartType, ksChartFamily, stack, semi_circle, ks_show_data_value, chart_data, item) {
			...
			*/
			...
			if (ksChartFamily == "circle") {
				...
				options.scales.yAxes[0].gridLines.display = item.ks_show_gridlines;
				...
				else if (chartType === "ks_bar_chart_many") {
					options.scales.yAxes[0].ticks.callback = function(value, index, values) {
						if (chart_data.ks_monetary_unit_1_many) {
							var ks_currency_id = chart_data.ks_monetary_unit_1_many;
							var ks_data = self.ksNumFormatterCustom(value, 1);
							ks_data = KsGlobalFunction.ks_monetary(ks_data, 105);
							return ks_data;
						} else {
		...
		_onKsWidgetOptionsClick: function() {
			var widgets = document.getElementsByClassName('grid-stack-item')

			for(let i=0; i<widgets.length; i++){
				let item_header = widgets[i].getElementsByClassName("ks_dashboard_item_header")[0]
				let item_header_l2 = widgets[i].getElementsByClassName("ks_dashboard_item_header_l2")[0]
				let item_header_l6 = widgets[i].getElementsByClassName("ks_dashboard_item_header_l6")[0]
				if(item_header){
					var header = item_header
				}
				else if(item_header_l2){
					var header = item_header_l2
				}
				if(item_header_l6){
					var header = item_header_l6
				}
				if(header.classList.contains('ks_widget_hide')){
					header.classList.remove('ks_widget_hide')
				}else{
					header.classList.add('ks_widget_hide')
				}
			}
		},

		_onKsItemHover_l2:function(e){
			var self = this;
			var id = parseInt($($(e.currentTarget).parentsUntil('.grid-stack').slice(-1)[0]).attr('data-gs-id'));
			var item_data = self.ks_dashboard_data.ks_item_data[id];
			
			
			var zoom = item_data.ks_zoom
			
			if(edit_options == false && zoom != 'off'){
				var self = this;
				...
				var div_item = document.getElementById('item_selected')
				
				var widget_width = item[0].offsetWidth
				var widget_height = item[0].offsetHeight
				var zoom_active = false
				
				
				var item_data = self.ks_dashboard_data.ks_item_data[id];
				
				var item_selected = item[0].cloneNode(true)

				switch (zoom) {
					case 'x2':
						item_selected.style.width = widget_width*2+"px"
						item_selected.style.height = widget_height*2+"px"
						zoom_active = true
						break;
					case 'x3':
						item_selected.style.width = widget_width*3+"px"
						item_selected.style.height = widget_height*3+"px"
						zoom_active = true
						break;
					case 'x4':
						item_selected.style.width = widget_width*4+"px"
						item_selected.style.height = widget_height*4+"px"
						zoom_active = true
						break;
					default:
				...
				if(first_widget==false){
					popup.style.top = "195px"
				}
				
				
				if(zoom_active ==true){
					div_item.append(item_selected)
		...
		_onKsItemHover_l5:function(e){
			...
			if(edit_options == false && zoom != 'off'){
				var self = this;
				var item = $($(e.currentTarget).parentsUntil('.grid-stack'))
				var id = parseInt($($(e.currentTarget).parentsUntil('.grid-stack').slice(-1)[0]).attr('data-gs-id'));
		...
		_onKsItemHover:function(e){
			...
			if(edit_options == false && zoom != 'off'){
				...
				var main_body =  item_selected.getElementsByClassName('ks_dashboard_item_main_body')[0]
				main_body.classList.remove('ks_dashboard_item_main_body')
				main_body.classList.add('ks_main_item')
				if(first_widget==false){
					popup.style.top = "195px"
				}
		...
		_onKsChartHover:function(e){
			...
			if(edit_options == false && zoom != 'off'){
				var chart_data = JSON.parse(item_data.ks_chart_data);
				var isDrill = item_data.isDrill ? item_data.isDrill : false;
				var chart_id = item_data.id,
					chart_title = item_data.name;
				var chart_title = item_data.name;
				var chart_type = item_data.ks_dashboard_item_type.split('_')[1];
				...
				switch (chart_type) {
					...
					case "polarArea":
						...
						var setsCount = chart_data.datasets[0].data.length;
					...
					case "area":
						...
						var setsCount = chart_data.datasets.length;
						
						break;
				}

				var gradient;
				switch (item_data.ks_chart_item_color) {
					case 'cool':
						gradient = {
							0: [255, 255, 255, 1],
							20: [220, 237, 200, 1],
				...
				if (item_data.ks_chart_item_color !== "default") {
					//Get a sorted array of the gradient keys
					var gradientKeys = Object.keys(gradient);
					gradientKeys.sort(function(a, b) {
						return +a - +b;
				...
				var chart_plugin = [];
				if (item_data.ks_show_data_value) {
					chart_plugin.push(ChartDataLabels);
				}
				...
				var item_copy = item[0].cloneNode(true)
				var card_body = item_copy.getElementsByClassName('card-body')[0]
				card_body.innerHTML=""
				card_body.classList.remove('card-body','ks_chart_card_body')
				card_body.style.margin = "0px 20px 0px 20px"
				var destinationCanvas = document.createElement('canvas');
				destinationCanvas.style.backgroundColor = "#FFF"
				card_body.appendChild(destinationCanvas)
				switch (zoom) {
					case 'x2':
						item_copy.style.width = widget_width*2+"px"
						item_copy.style.height = widget_height*2+"px"
						zoom_active = true
						break;
					case 'x3':
						item_copy.style.width = widget_width*3+"px"
						item_copy.style.height = widget_height*3+"px"
						zoom_active = true
						break;
					case 'x4':
						item_copy.style.width = widget_width*4+"px"
						item_copy.style.height = widget_height*4+"px"
						zoom_active = true
						break;
					default:
						break;
				...
				if(chart_type == "pie" || chart_type == "doughnut" || chart_type == "polarArea"){
					for (var i = 0; i < chart_data.datasets.length; i++) {
						chart_data.datasets[i].backgroundColor = chartColors;
						chart_data.datasets[i].borderColor = "rgba(255,255,255,1)";
					}
				}
				if(chart_type == "bar" || chart_type == "horizontalBar"){
					for (var i = 0; i < chart_data.datasets.length; i++) {
						chart_data.datasets[i].backgroundColor = chartColors[i];
						chart_data.datasets[i].borderColor = "rgba(255,255,255,1)";
					}
				}
				if(chart_type == "line" || chart_type == "area"){
					for (var i = 0; i < chart_data.datasets.length; i++) {
						if(chart_type == "line"){
							chart_data.datasets[i].backgroundColor = "transparent";
						}
						chart_data.datasets[i].borderColor = chartColors[i];
					}
				}

				var popup = document.getElementById('item_popup')
				var data = [chart_data.datasets]
				div_item.style.height = "100%"

				for(let i=0;i<chart_data.datasets.length;i++){
					if(chart_data.datasets[i].type == "pie" || chart_data.datasets[i].type == "doughnut" || chart_type == "polarArea"){

						chart_data.datasets[i].backgroundColor = chartColors;
						chart_data.datasets[i].borderColor = "rgba(255,255,255,1)";

					}
					if(chart_data.datasets[i].type == "bar" || chart_data.datasets[i].type == "horizontalBar"){
						
						chart_data.datasets[i].backgroundColor = chartColors[i];
						chart_data.datasets[i].borderColor = "rgba(255,255,255,1)";
						
					}
					if(chart_data.datasets[i].type == "line" || chart_data.datasets[i].type == "area"){
						
							if(chart_data.datasets[i].type == "line"){
								chart_data.datasets[i].backgroundColor = "transparent";
							}
							chart_data.datasets[i].borderColor = chartColors[i];
					...
					data[i]= {
						type:chart_data.datasets[i].type,
						label:chart_data.datasets[i].label,
						data:chart_data.datasets[i].data,
						backgroundColor:chart_data.datasets[i].backgroundColor,
						borderColor: chart_data.datasets[i].borderColor
					}
				}

				if(zoom_active == true){
					div_item.append(item_copy)
					var ksMyChart = new Chart(destinationCanvas, {
						type: chart_type === "area" ? "line" : chart_type,
						plugins: chart_plugin,
						data: {
							labels: chart_data['labels'],
						...
						options: {
							...
							plugins: {
								datalabels: {
									...
									backgroundColor: function(context) {
										// return context.dataset.borderColor
										return context.dataset.backgroundColor !='transparent'? context.dataset.backgroundColor:context.dataset.borderColor ;
									},
									borderRadius: 4,
									color: 'white',
									font: {
