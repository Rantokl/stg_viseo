odoo.define('ks_dashboard_ninja.ks_dashboard', function(require) {
    ...
    var KsDashboardNinja = AbstractAction.extend({
        ...
        start: function() {
            ...
            console.log(this.$el[0].style.background = "black")
            return this._super();
        },
                ...
                if (self.grid) {
                    if (items[i].ks_dashboard_item_type === 'ks_tile') {
                        ...
                        if (items[i].id in self.gridstackConfig) {
                            self.grid.addWidget($(item_view), self.gridstackConfig[items[i].id].x, self.gridstackConfig[items[i].id].y, self.gridstackConfig[items[i].id].width, self.gridstackConfig[items[i].id].height, false, 3, null, 1, 2, items[i].id);
                        } else {
                            self.grid.addWidget($(item_view), 0, 0, 8, 2, true, 3, null, 1, 2, items[i].id);
                        }
                        ...
                        if (items[i].id in self.gridstackConfig) {
                            self.grid.addWidget($kpi_preview, self.gridstackConfig[items[i].id].x, self.gridstackConfig[items[i].id].y, self.gridstackConfig[items[i].id].width, self.gridstackConfig[items[i].id].height, false, 3, null, 1, 3, items[i].id);
                        } else {
                            self.grid.addWidget($kpi_preview, 0, 0, 3, 2, true, 3, null, 1, 3, items[i].id);
                        }
        ...
        _onKsChartHover:function(e){
            ...
            if(edit_options == false && zoom != 'off'){
                ...
                // }]
                // data1.push(chart_data.datasets,chart_data.datasets)
                // console.log(data)
                // console.log(data)
                if(zoom_active == true){
