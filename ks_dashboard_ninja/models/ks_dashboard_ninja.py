...
class KsDashboardNinjaBoard(models.Model):
    ...
    def default_get(self, fields):
        ...
        if 'ks_dashboard_top_menu_id' in fields:  # avoid forcing the partner of the first lead as default
            menu_root = self.env.ref('ks_dashboard_ninja.board_menu_root', raise_if_not_found=False)
            if bool(menu_root):
                res['ks_dashboard_top_menu_id'] = self.env.ref('ks_dashboard_ninja.board_menu_root', raise_if_not_found=False).id
        return res
    ...
    ks_dashboard_user_access = fields.Many2many('res.users', string ="Users Access")

    ks_dashboard_background = fields.Char(string="Color",default="#8f8f8f,0.99")
    ...
    ks_dashboard_default_template = fields.Many2one('ks_dashboard_ninja.board',
    ...
    def create(self, vals):
        ...
        if 'ks_dashboard_top_menu_id' in vals and 'ks_dashboard_menu_name' in vals:
            ...
            record.ks_dashboard_menu_id = self.env['ir.ui.menu'].sudo().create({
                ...
                'parent_id': vals.get('ks_dashboard_top_menu_id'),
    ...
    @api.model
    def get_dashboard_color(self,id):
        
        return self.env['ks_dashboard_ninja.board'].browse(id).mapped('ks_dashboard_background')[0]
