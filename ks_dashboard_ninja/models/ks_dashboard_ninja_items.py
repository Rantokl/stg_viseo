...
class KsDashboardNinjaItems(models.Model):
    ...
    # "Name"
    name = fields.Char(string="Nom du widget", size=256)
    # "Model"
    ...
    #"Domain"
    ks_domain = fields.Char(string="Domaine")
    ...
    #"Background Color"
    ks_background_color = fields.Char(string="Couleur Arrière Plan",
                                      default="#ffffff,0.99")
    ks_icon = fields.Binary(string="Upload Icon", attachment=True)
    ks_default_icon = fields.Char(string="Icon", default="bar-chart")
    # "Icon Color"
    ks_default_icon_color = fields.Char(default="#ffffff,0.99", string="Couleur Icon")
    # Icon Option
    ks_icon_select = fields.Char(string="Choisir Icon", default="Default")
    #"Font Color"
    ks_font_color = fields.Char(default="#ffffff,0.99", string="Couleur Text")
    ks_dashboard_item_theme = fields.Char(string="Theme", default="white")
    # "Zoom"
    ks_zoom = fields.Selection([('off','Off'),
                                ('x2','x2'),
                                ('x3','x3'),
                                ('x4','x4'),
                            ],default=('off'),string="Zoom")
    ...
    ks_layout = fields.Selection([('layout1', 'Layout 1'),
                                  ...
                                  ('layout5', 'Layout 5'),
                                  ('layout6', 'Layout 6'),
                                  ], default=('layout1'), required=True, string="Style")
    ks_preview = fields.Integer(default=1, string="Preview")
    ks_model_name = fields.Char(related='ks_model_id.model', string="Model Name")

    # "Count,Sum,Average"
    ks_record_count_type_2 = fields.Selection([('count', 'Nombre'),
                                               ('sum', 'Somme'),
                                               ('average', 'Moyenne')], string="Kpi Record Type", default="sum")
    ...
    # "Record Type","Count","Sum","Average"
    ks_record_count_type = fields.Selection([('count', 'Nombre'),
                                             ('sum', 'Somme'),
                                             ('average', 'Moyenne')], string="Type", default="count")
    ...
    ks_date_filter_field = fields.Many2one('ir.model.fields',
                                           domain="[('model_id','=',ks_model_id),'|',('ttype','=','date'),"
                                                  "('ttype','=','datetime')]",
                                           string="Filtrer par Date")
    #"Date Filter Selection"
    ks_date_filter_selection = fields.Selection([
        ('l_none', 'Aucune'),
        ('l_day', 'Aujourd"hui'),
        ('t_week', 'Cette semaine'),
        ('t_month', 'Ce mois'),
        ('t_quarter', 'Ce trimestre'),
        ('t_year', 'Cette année'),
        ('n_day', 'Demain'),
        ('n_week', 'Semaine suivante'),
        ('n_month', 'Mois suivant'),
        ('n_quarter', 'Trimestre suivant'),
        ('n_year', 'Année suivante'),
        ('ls_day', 'Hier'),
        ('ls_week', 'Semaine dernière'),
        ('ls_month', 'Mois dernier'),
        ('ls_quarter', 'Trimestre dernier'),
        ('ls_year', 'Année dernière'),
        ('l_week', '7 derniers jours'),
        ('l_month', '30 derniers jours'),
        ('l_quarter', '90 derniers jours'),
        ('l_year', '365 derniers jours'),
        ('l_custom', 'Filtre personnaliser'),
    ], string="Filtre appliquée", default="l_none", required=True)

    #"Start Date","End Date"
    ks_item_start_date = fields.Datetime(string="Date début")
    ks_item_end_date = fields.Datetime(string="Date fin")
    ...
    # "Data Type"
    ks_chart_data_count_type = fields.Selection([('count', 'Nombre'), ('sum', 'Somme'), ('average', 'Moyenne')],
    ...
    ks_sort_by_field = fields.Many2one('ir.model.fields',
                                       ...
                                       string="Trié par")
    # "Sort Order"
    ks_sort_by_order = fields.Selection([('ASC', 'Ascendant'), ('DESC', 'Descendant')],
                                        string="Ordre de Tri")
    # "Record Limit"
    ks_record_data_limit = fields.Integer(string="Limit")
    ...
    ks_chart_item_color = fields.Selection(
        ...
        string="Palette de couleurs", default="default")
    ...
    #"Company"
    ks_company_id = fields.Many2one('res.company', string='Société', default=lambda self: self.env.user.company_id)
    ...
    # "Show Data Value"
    ks_show_data_value = fields.Boolean(string="Afficher valeur")
    ...
    # "Show Custom Unit"
    ks_unit = fields.Boolean(string="Autre unité", default=False)
    ks_unit_selection = fields.Selection([
        ('monetary', 'Monétaire'),
        ('custom', 'Personnalisé'),
    ], string="Unité appliqué")
    ks_chart_unit = fields.Char(string="Entrer Unité", size=5, default="",
                                help="Maximum 5 caractères, ex: km, m")

    # User can stop propagation of the tile item
    #"Show Records"
    ks_show_records = fields.Boolean(string="Voir Enregistrements", default=True, help="""This field Enable the click on 
    ...
    def ks_fetch_drill_down_data(self, item_id, domain, sequence):
        ...
        if action_line.ks_chart_type == 'ks_list_view':
            ...
            if action_line.ks_item_action_field.ttype == 'many2one':
                ...
                ks_list_view_data['list_view_type'] = 'date_type'
                ks_list_field = []
                ks_list_view_data['groupby'] =  action_line.ks_item_action_field.name  + ':' + action_line.ks_item_action_date_groupby
