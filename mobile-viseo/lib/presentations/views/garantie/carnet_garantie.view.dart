...
import 'package:sav/presentations/controllers/garantie/carnet_garantie.controller.dart';
...
class CarnetGarantieView extends BaseStatelessView<CarnetGarantieController> {
   CarnetGarantieView({
    Key? key,
  }) : super(key: key, controller: Get.put(CarnetGarantieController())){
     WidgetsBinding.instance!.addPostFrameCallback((_) {
       controller.getCarnetGarantie(
           success: (success) {
  ...
  void download() async {
      ...
      String fileName = "${controller.vehicleSelected?.number}_carnet_garantie.pdf";
      String filePath = path.join(downloadDirectory.path, fileName);

      // Load the PDF asset
      // Download the PDF file
      dynamic response = await dio.get(controller.carnetGarantie.value!.data.first.pdf, options: Options(responseType: ResponseType.bytes));
  ...
  Widget build(BuildContext context) {
    ...
    return baseScaffoldView(
      appBarController: AppBarController(
        title: Strings.garantie.carnetGarantieTitle,
      ),
      withHeader: true,
      body: Obx(() => controller.isLoading
          ? Container()
          : controller.carnetGarantie.value == null ? Expanded(
                  child: Container(
                    alignment: Alignment.center, // Center the text both vertically and horizontally
                    child: Text(
                      Strings.garantie.noPdf,
                        ...
                        Padding(
                          ...
                          child: SizedBox(
                            ...
                            child: SfPdfViewer.network(
                              controller.carnetGarantie.value!.data.first.pdf,
                              pageSpacing: 40,
                              key: _pdfViewerKey,
                            ),
                          ),
                          ...
                          child: Column(
                            children: [
                              TextButton(
                                ...
                                onPressed: () => download(),
                                child: Text(Strings.garantie.downloadPdf),
                              ),
