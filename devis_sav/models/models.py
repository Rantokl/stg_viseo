...
class SaleOrderPDFView(models.TransientModel):
    ...
    quotation_pdf = fields.Binary(string='Devis PDF', filters='.pdf', compute='take_pdf', related='ir_attach.datas',
                                  default=lambda self: self.ir_attach.datas)
    ...
    def save_pdf_to_server(self):
        ...
        if report:
            ...
            sale_order_id = self.sale_order_id
            if sale_order_id.repair_id:
                curs, connex = dbconnex(self)
                query = """
                        INSERT INTO public."viseoApi_devis"(details, owner_id, type_devis_id, vehicle_id, date_devis, status_devis_id, uploaded_at,created_at)
            	        VALUES (%s, %s, %s, %s, %s, %s, %s,%s) RETURNING id;
            	        """
                value = (sale_order_id.repair_id.reception_comments, sale_order_id.repair_id.vehicle_id.driver_id.id, 7,
                         sale_order_id.repair_id.vehicle_id.id, datetime.datetime.now(), 1, datetime.datetime.now(),datetime.datetime.now(),)

                record_id = self.insertData(query, value)

                devis_demand = {
                    'customer_vehicle_id':sale_order_id.repair_id.vehicle_id.id,
                    'customer_id':sale_order_id.partner_id.id,
                    # 'type_devis':3,
                    'date_devis':datetime.datetime.now().date(),
                    'note':sale_order_id.repair_id.reception_comments,
                    'devis_id':record_id[0]
                }
                demand=self.env['sale.order.demand'].create(devis_demand)
                sale_order_id.write({'demand_devis': demand.id})
                date_devis = self.sale_order_id.demand_devis.date_devis
                ...
                curs.execute("""
                                ...
                                WHERE id = %s;
                """, (sale_number, total, pdf_name, datetime.datetime.now(), record_id[0]))
                connex.commit()
                curs.close()
                connex.close()
                ...
                #     curs, connex = database.dbconnex(self)
                #     query = """
                #                             INSERT INTO public."viseoApi_devis"(details, owner_id, type_devis_id, vehicle_id, date_devis,  status_devis_id,  created_at)
                #             	                                VALUES (%s, %s, %s, %s, %s, %s, %s) RETURNING id;"""
                #     value = (sale_order_id.repair_id.reception_comments, sale_order_id.partner_id.id, 3,
                #              sale_order_id.repair_id.vehicle_id.id, datetime.datetime.now(), 1,
                #              datetime.datetime.now(),)
                #
                #     record_id = self.insertData(query, value)
                #     sale_order_id.write({'demand_devis': record_id[0]})
                #     devis_demand = {
                #         'customer_vehicle_id': sale_order_id.repair_id.vehicle_id.id,
                #         'customer_id': sale_order_id.partner_id.id,
                #         'type_devis': 3,
                ...
                #         'devis_id': record_id[0]
                #     }
                #     self.env['sale.order.demand'].create(devis_demand)
                date_devis = self.sale_order_id.demand_devis.date_devis
                sale_number = sale_order_id.name
                demand_id = sale_order_id.demand_devis.devis_id
                total = sale_order_id.amount_total
    ...
    def check_sale_order_validate(self):
        ...
        for data in datas:
            ...
            if not devis:
                ...
                print(sale_order)
                if not sale_order:
                    print('Sale order doesnt exist')
                else:
                    devis.write(records)
                    sale_order.message_post(
                        body='Devis validé par le client via l\'application mobile',
                        subject="{}".format(sale_order.name),
                        partner_ids=sale_order.create_uid.partner_id.ids
                    )
    ...
    def check_devis_apk(self):
        ...
        for row in devis:
            ...
            ask = devis.message_post(
                body='''Demande de devis de Mr(s) {} du {}'''.format(devis.customer_id.name, devis.date_devis),
